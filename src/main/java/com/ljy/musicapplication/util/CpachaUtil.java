...
public class CpachaUtil {
     ...
     * 字体
     */
    final private String[] fontNames = new String[] { "黑体", "宋体", "Courier", "Arial", "Verdana", "Times", "Tahoma",
            "Georgia" };
    /**
     * 字体样式
     */
     ...
     * generateCpacha(
     *
     * @RequestParam(name="vl",required=false,defaultValue="4") Integer
     * vcodeLen,
     *
     ...
     *
     * @RequestParam(name="type",required=true,defaultValue="loginCpacha")
     * String cpachaType, HttpServletRequest request, HttpServletResponse
     * response){ CpachaUtil cpachaUtil = new CpachaUtil(vcodeLen, width,
     * height); String generatorVCode = cpachaUtil.generatorVCode();
     * //将验证码保存在session中，在后台传过来的时候可以从session中取出来验证
     * request.getSession().setAttribute(cpachaType, generatorVCode);
     * BufferedImage generatorRotateVCodeImage =
     * cpachaUtil.generatorRotateVCodeImage(generatorVCode, true); try {
     * ImageIO.write(generatorRotateVCodeImage, "gif",
     * response.getOutputStream()); } catch (IOException e) { // TODO
     * Auto-generated catch block e.printStackTrace(); } }
     */

	/*jsp中使用：
	<img alt="验证码加载失败" src="${pageContext.request.contextPath}/get_cpacha" width="100" height="30" id="Img"
